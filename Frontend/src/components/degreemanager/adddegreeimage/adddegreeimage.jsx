...
const AddDegreeImage = () => {
  ...
  // Lấy danh sách đơn vị cấp (issuers)
  useEffect(() => {
    const fetchIssuers = async () => {
      try {
        ...
        console.log('Danh sách đơn vị cấp:', response.data);
        ...
        console.error('Lỗi khi lấy danh sách đơn vị cấp:', err);
  ...
  // Lấy danh sách loại văn bằng (degree types) khi chọn issuer
  useEffect(() => {
    if (selectedIssuer) {
      const fetchDegreeTypes = async () => {
        try {
          ...
          console.log('Danh sách loại văn bằng:', response.data);
          if (response.data.errCode === 0) {
          ...
          console.error('Lỗi khi lấy danh sách loại văn bằng:', err);
  ...
  const handleUploadSingle = async () => {
    ...
    try {
      ...
      console.log('Kết quả trích xuất:', response.data);
      if (response.data.errCode === 0) {
    ...
    try {
      ...
      for (const file of folderFiles) {
        ...
        try {
          ...
          console.log(`Kết quả trích xuất cho ${file.name}:`, response.data);
          if (response.data.errCode === 0) {
      ...
      console.error('Lỗi khi tải lên thư mục:', err.response?.data || err.message);
